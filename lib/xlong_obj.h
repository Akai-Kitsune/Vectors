...
/// A macro function to check if a large number is equal to zero.
#define XLN_ISZERO(number) (number->_current == 1 && number->_mem[0] == 0)

/// The function assigns one to the number.
#define XLN_SETONE(number) number->_current = 1; number->_mem[0] = 1; number->_sign = 1;

/// The function assigns zero to the number.
#define XLN_SETZERO(number) number->_current = 1; number->_mem[0] = 0; number->_sign = 1;

/// The function changes the sign of the number to negative.
#define XLN_SIGNN(number) number->_sign = -1;

/// The function changes the sign of the number to positive.
#define XLN_SIGNP(number) number->_sign = 1;
